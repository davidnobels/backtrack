...
public class BackTrackAlgorithm {
    ...
    int iterator = 1;
    static int jumlahIterasi, jumlahSolusi, jumlahSimpulMati, jumlahAnak;
...
public BackTrackAlgorithm(int nQueen){
    ...
    jumlahIterasi = jumlahSolusi = jumlahSimpulMati = jumlahAnak = 0;
...
private boolean check(int i, int j){
    jumlahIterasi++;
    if (i == j || (hasil[i] != hasil[j] && Math.abs(i - j) != Math.abs(hasil[i] - hasil[j]) && check (i + 1, j) && (hasil[j] > 0 && hasil[i] <= nQueen))){
        return true;
    }else {
        return false;
    }
...
public int[] routeUseBackTrack() {
    ...
    if (check(1, iterator) && hasil[iterator] <= nQueen) {
        cleanRoute(iterator + 1);
        iterator++;
        jumlahIterasi++;
    } else {
